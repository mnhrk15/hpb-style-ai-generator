...
- **髪だけを変える「Fill（部分編集）」機能追加**
    - FLUX.1 Fill APIを活用し、顔や背景を変えずに髪型・髪色だけを編集可能
    - 画像上で手動マスキング（ブラシで髪部分を塗る）UIを実装
    - ブラシサイズ変更・Undo・リセット・スマホ対応
    - 生成モード切替UI（「かんたん生成（マスクなし）」/「こだわり生成（マスクあり）」）を追加
    - 画像アップロード・URL取得どちらでもマスキング可能
    - 生成時にmode, mask_dataをAPIに送信し、Kontext/Fillを自動切替

### 🖌️ マスキングUIの使い方

1. 画像をアップロードまたはURLで取得
2. 「こだわり生成（マスクあり）」を選択
3. 画像上で髪部分を白く塗る（ブラシサイズ調整可、Undo/リセット可）
4. プロンプトを入力し生成

### ⚠️ 注意点・UX改善案
- Tailwind CDNは本番ではビルド型推奨（開発中は警告が出ますが無視可）
- ObjectURLはアップロード画像の解放を推奨
- スマホ・タブレットでもマスキング操作可
- 画像リサイズ時はcanvasも再調整
- Toastifyによる通知UIでエラー・成功を即時表示
